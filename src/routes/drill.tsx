...
function DrillPage() {
  ...
  return <Page narrow><PageTitle eyebrow="Build your session" title="Custom Drill" subtitle="Choose what to focus on. You can change any setting before starting." />
    <div className="space-y-4">
      ...
      <Surface><Step n="3" title="Choose Material" /><div className="grid gap-2 sm:grid-cols-2">{availableMaterials.map((item) => { const active = selected.includes(item.id); return <label key={item.id} className={cn("flex cursor-pointer items-center gap-3 rounded-xl border p-3.5 transition-colors", active ? "glass-pill-active" : "")}><Checkbox checked={active} onCheckedChange={() => toggle(item.id)} /><span className="text-sm font-medium">{item.name}</span></label>; })}</div></Surface>
